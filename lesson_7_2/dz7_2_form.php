<?php
        require ("dz7_2_data.php");
?>

<title>Купи слона</title>
<h1 align="center"><strong>Доска объявлений</strong></h1>
<form style="margin-left: 30%;" method="post"> <font size="4">
    <div> <label class="form-label-radio"><input type="radio" value="1" checked="" <?php echo $print_ad['privat'] == 1 ? 'checked' : '' ?> name="privat">Частное лицо</label> <label class="form-label-radio"><input type="radio" value="2" <?php echo $print_ad['privat'] == 2 ? 'checked' : '' ?> name="privat">Компания</label> </div>
    ...
    <div> <label for="allow_mails"> <input type="checkbox" value="checked" <?php echo isset($print_ad['allow_mails']) ? $print_ad['allow_mails']:''; ?> name="allow_mails" id="allow_mails"><span>Я не хочу получать вопросы по объявлению по e-mail</span> </label> </div>
    ...
    <input type="submit" value="<?php echo isset($_GET['click_id']) ? 'Сохранить' : 'Добавить' ?> объявление" id="form_submit" name="confirm_add"> <input type="submit" value="Очистить форму" id="form_submit" name="clear_form">
    <div><input <?php echo isset($_GET['click_id']) ? '' : 'hidden=""' ?> type="submit" value="Назад" id="enter_id" name="back"> <input type="submit" value="Очистить базу объявлений" id="clear_base" name="clear_base"></div>
