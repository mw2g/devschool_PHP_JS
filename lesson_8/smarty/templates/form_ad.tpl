...
<form style="margin-left: 30%;" method="post"> <font size="4">
    <div> 
        <label class="form-label-radio">
            <input type="radio" value="1" checked="" {if $display.privat eq 1}checked{/if} name="privat">Частное лицо
            <input type="radio" value="2" {if $display.privat eq 2}checked{/if} name="privat">Компания
        </label>
    </div>
    ...
    <div>
        <label for="allow_mails"> <input type="checkbox" value="checked" {if isset($display.allow_mails)}checked{/if} name="allow_mails" id="allow_mails">
    ...
    <div class="form-group">
        ...
        {html_options name=location_id options=$citys selected=$display.location_id}
    </div>
    <div class="form-group">
        <label for="fld_category_id" class="form-label">Категория</label> 
        {html_options name=category_id options=$categorys selected=$display.category_id}
    </div> 
    <div id="f_title">
